...
async def nerd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in [ChatType.GROUP, ChatType.SUPERGROUP]:
        ...
        if not settings.get('ai_enabled', True):
            await update.message.reply_text("❌ AI is disabled in this group.")
            return
    else:
    ...
    if result['success']:
        ...
        try:
            ...
            try:
                ...
                sent_message = await update.message.reply_text("❌ Error sending response. Please try again.")
                return
...
async def pin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    notify = True
    if context.args and 'silent' in context.args[0].lower():
        notify = False
...
async def provider_callback(query, context, data):
    ...
    settings = db.get_user_settings(user.id) or {}
    current_model = settings.get(f'{provider}_model', models[0] if models else "")
...
def main():
    ...
    # Start the bot with health check
